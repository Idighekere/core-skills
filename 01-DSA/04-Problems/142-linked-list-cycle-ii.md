...
## Optimized Solution
This solution has a mathematical proof that when we traverse and both `slow` and `fast` meet, we should now iterate the `head` and the `slow` at same speed and that the points where `slow` and `head` meet is the point that the cycle started. 
## Mistakes
